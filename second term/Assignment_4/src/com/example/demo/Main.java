...
public class Main extends Application {
    ...
    public void stop(){
        String backup=new File(readupstr+"\\resources\\assets\\data\\backup.dat").toString();
        try {
    ...
    public void start(Stage primaryStage) throws IOException, ClassNotFoundException {
      films=new Films();        data=FXCollections.observableArrayList();
        IconImage=new Image(new File(readupstr+"\\resources\\assets\\icons\\logo.png").toURI().toURL().toString());
        primaryStage.getIcons().add(IconImage);
        users=new Users();
     //   final URL resource = getClass().getResource("error.mp3");
       Media sound = new Media(new File(readupstr+"\\resources\\assets\\effects\\error.mp3").toURI().toURL().toString());
    ...
    private  void readbackup() throws IOException, ClassNotFoundException {
      ...
      //   bf = new BufferedInputStream (new FileInputStream("news.dat"));
       String str=new File(readupstr+"\\resources\\assets\\data\\backup.dat").toString();
