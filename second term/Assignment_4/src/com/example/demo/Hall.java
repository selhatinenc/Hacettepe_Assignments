...
public class Hall {
    ...
    public VBox setSeats() throws MalformedURLException {
        label2.setText("");
         ...
         vBox=new VBox();
        Image  image = new Image(new File(".\\resources\\assets\\icons\\empty_seat.png").toURI().toURL().toString(),false);
       Image full= new Image(new File(".\\resources\\assets\\icons\\reserved_seat.png").toURI().toURL().toString());
        for (int i = 0; i < row; i++) {
            HBox hBox =new HBox();
            for (int j = 0; j <column ; j++) {
                ImageView imageview;
                ...
                imageview.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<Event>() {
                    ...
                    public void handle(Event event) {
                        if (seats[finalI ][finalJ ].isFree()){
                            ...
                            try {
                                imageview.setImage(new Image(new File(".\\resources\\assets\\icons\\reserved_seat.png").toURI().toURL().toString()));
                            ...
                            try {
                                imageview.setImage(new Image(new File(".\\resources\\assets\\icons\\empty_seat.png").toURI().toURL().toString()));
                            } catch (MalformedURLException e) {
                                e.printStackTrace();
                            }
